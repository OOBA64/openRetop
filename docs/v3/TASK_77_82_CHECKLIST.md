...
checks, compileall, diff check, status update, and task commit pass.
...
- [x] Run and record the existing MainWindow compatibility-suite baseline; failures are
  isolated to the pre-existing compatibility adapter and carried forward as integration debt.
...
- [x] Add boundary tests; legacy MainWindow orchestration remains a compatibility adapter
  until the Qt workflow is complete.
